...
def generation_size(sample_size, gen_exp):
    gens = np.array(
        list(
            map(
                lambda x: int(2 * (gen_exp ** x)),
                range(int(np.log(sample_size * gen_exp) / np.log(gen_exp))),
            )
        )
    )
    enough_gens = np.where(np.cumsum(gens) > sample_size)[0][0]
    return gens[:enough_gens].tolist() + [
        sample_size - gens[:enough_gens].sum()
    ]
...
def households(generation, prev_generation):
    ...
    half_gen = int(gen_size / 2)
    single_parents = [
        [x] for x in np.random.choice(generation, int(gen_size * 0.32))
    ]
    ...
    mix_generations = list(
        zip(
            np.random.choice(
                generation[:half_gen], int(gen_size * 0.2 * 0.68 * 0.5)
            ),
            np.random.choice(
                prev_generation[half_prev_gen:],
                int(gen_size * 0.2 * 0.68 * 0.5),
            ),
        )
    )
    mix_generations += list(
...
def combine_ind_to_households(generations, remove_rate):
    ...
    for i in range(1, len(generations)):
        hh = households(
            generations[i - 1], None if i == 1 else generations[i - 2]
        )
        trios = zip(
            generations[i],
            hh[np.random.choice(hh.shape[0], generations[i].size)],
        )  # hh[]
        child_parent = [
            [[child, parent] for parent in parents] for child, parents in trios
        ]
    ...
    return all_child_parent[
        np.random.choice(total_edges, int(total_edges * remove_rate))
    ]
...
def count_with_removed_edges(
    edges_remove_part, rel_matrix, edges, wanted_number_of_ibds
):
...
def find_number_of_edges_to_remove(rel_matrix, edges, wanted_number_of_ibds):
    try:
        res = fmin(
            count_with_removed_edges,
            0.3,
            args=(rel_matrix, edges, wanted_number_of_ibds,),
        )
        if res > 0:
            return None
...
def simulate_tree(sample_size, sparse_factor, gen_exp, init_keep_rate):
    ...
    res = find_number_of_edges_to_remove(
        rel_matrix, edges, wanted_number_of_ibds
    )
    if res == None:
        ...
        assert (
            np.abs(count_IBD_nonzero(rel_matrix) - wanted_number_of_ibds)
            < 0.1 * wanted_number_of_ibds
        )
