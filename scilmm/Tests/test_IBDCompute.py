...
class TestIBDCompute(TestCase):
    ...
    def test_simulate_relationship(self):
        pedigree, entries_list = self.ibd_compute.simulate_relationship(
            sample_size=1000, sparsity_factor=0.01
        )
        ...
        self.assertRaises(
            Exception,
            self.ibd_compute.simulate_relationship,
            sample_size=100,
            sparsity_factor=0.0001,
        )

    def test_compute_ibd(self):
        _, _ = self.ibd_compute.simulate_relationship(
            sample_size=1000, sparsity_factor=0.01
        )
    ...
    def test_half_and_full_brothers(self):
        ...
        self.assertEqual(
            ibd[
                (
                    np.argwhere(entries_list == "0_9")[0][0],
                    np.argwhere(entries_list == "0_8")[0][0],
                )
            ],
            0.25,
        )
        self.assertEqual(
            ibd[
                (
                    np.argwhere(entries_list == "0_3")[0][0],
                    np.argwhere(entries_list == "0_4")[0][0],
                )
            ],
            0.5,
        )
